...
std::vector<std::string> fleur::Filter::filterHTMLByMarked(const std::string &strToFilter, const std::string &markerStr) {
    ...
    if (!(std::find(markerHtml.begin(), markerHtml.end(), markerBuff) != markerHtml.end()))
        throw fleur::FilterException("No valid html marker");
